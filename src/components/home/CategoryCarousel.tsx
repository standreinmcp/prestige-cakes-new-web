        ...
        <div className="relative mt-16">
          ...
          <div className="grid gap-[34px] sm:grid-cols-2 xl:grid-cols-4">
            {categories.map((category) => (
              <article
                key={category.slug}
                className={`group flex flex-col ${interactiveCardShell()}`}
              >
                ...
                <div className="flex flex-col gap-2 px-3 py-5">
                  ...
                  <Link
                    href={`/vitrina-live?categorie=${category.slug}`}
                      className="inline-flex items-center gap-2 py-2.5 text-base font-medium text-brand-navy transition-all duration-300 group-hover:gap-3 group-hover:text-brand-gold"
                  >
                    Vezi tot
                    <ArrowRightIcon />
                  </Link>
